...
void MTCNN::detection(const cv::Mat& img, std::vector<cv::Rect>& rectangles)
{
    ...
    feature_get();
//vector<float> t1 = feature_map_[0];
//    vector<float> t2 = feature_map_[1];
//    float result = cosine_similarity(t1, t2);
    create_face_order();
//    img_prev_ = img_.clone();
...
void MTCNN::feature_get()
{
    ...
    input_layer->Reshape(cur_imgs.size(), num_channels_, input_geometry_[2].height, input_geometry_[2].width);
    int num = input_layer->num();
    /* Forward dimension change to all layers. */
    net_feature->Reshape();
    std::vector<cv::Mat> input_channels;
    WrapInputLayer(cur_imgs, &input_channels, 2);
    net_feature->Forward();
...
//        const shared_ptr<Blob<float> > feature_blob = net_feature->blob_by_name("conv6-3");
    ...
    //net_feature->Forward(input_vec);                                 // feed forward
//    const Dtype *feature_blob_data;
//    const shared_ptr<Blob<Dtype> > feature_blob = net_feature->blob_by_name("conv6-3");
//    int batch_size = feature_blob->num();
//    int dim_features = feature_blob->count() / batch_size;
//        //std::ofstream output_file("/home/xileli/Documents/video/RossVideo/output/feature.txt");
//
...
//        feature_map_.push_back(one_feature);
 ...
 //   }
    ...
    //try the feature ends

}

vector<vector<float>> MTCNN::vector_copy(vector<vector<float>> a) {
    vector<vector<float>> b_tmp(a.size(), vector<float>(a[0].size()));
    for (int i = 0; i < a.size(); i++) {
        vector<float> feature(a[i].size());
...
{
    vector<cv::Rect> bonding_box = bounding_box_;
    vector<vector<float>> feature_map = feature_map_;
    vector<vector<float>> feature_list;
    vector<int> name_list;
    int num = feature_map.size();
    float feature_result;
    if(num_bounding_box_ == 0 && !bonding_box.empty())  //feature list initialize
    {
        //feature_list = vector_copy(feature_map);
        feature_list = feature_map;
    }
    else
        feature_list = feature_list_;
    int feature_list_num = feature_list.size();
    for(int i = num - 1; i >= 0 ; i--)
    {
        ...
        {
//            vector<float> t1 = feature_list[j];
//            vector<float> t2 = feature_map[i];
//            float result = cosine_similarity(t1, t2);
            float sim = cosine_similarity(feature_list[j], feature_map[i]);
            if(sim >= similarity_threshold) {
                //feature_result = cosine_similarity(feature_list[j], feature_map[i]);
                //name_list.push_back(j);
                name_list_.push_back(j);
                statue =  true;
                break;
            }
        }
        if(!statue) {
            feature_list_.push_back(feature_map[i]);
            feature_list_num += 1;
            name_list_.push_back(feature_list_num);
        }
        feature_map.pop_back();
    }
//    for(int i = name_list.size(); i > 0 ; i--) {
//        name_list_.push_back(name_list[i]) ;
//    }
    if(!feature_map.empty()) std::cout<< "feature map has elements"<< std::endl;
    feature_list_ = feature_list;
    feature_map_ = feature_map;
    num_bounding_box_ = bounding_box_.size();

}

bool MTCNN::Check_scene_change()
{
    if(img_prev_.empty() || img_.empty()) return 0;
    cv::Mat img = img_.clone();
    cv::Mat img_prev = img_prev_.clone();
    if(img_prev.empty() || img.empty()) return 0;
    cv::Mat gray_diff;
    cv::Mat gray;
    cv::Mat gray_prev;
    int count = 0;
    cv::cvtColor(img_, gray, CV_BGR2GRAY);
    cv::cvtColor(img_prev, gray_prev, CV_BGR2GRAY);
    subtract(gray_prev, gray, gray_diff);

    for (int i = 0; i < gray_diff.rows; i++) {
        for (int j = 0; j < gray_diff.cols; j++) {
            if (abs(gray_diff.at<uchar>(i, j)) >= threshold_scene_diff_) {
                gray_diff.at<uchar>(i, j) = 255;
                count++;
            } else gray_diff.at<uchar>(i, j) = 0;
        }
    }

    imshow("foreground", gray_diff);
    cv::waitKey(5);
    return (count < gray_diff.rows * gray_diff.cols / 2);
